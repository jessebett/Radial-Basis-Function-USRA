...
bgcol=(0.921875,0.890625,0.84375)
fgcol=(0.545, 0.471, 0.435)
...
def makefinegrid():
    ...
    mlab.figure(fgcolor=fgcol,bgcolor=bgcol)
    mlab.mesh(coor.x, coor.y, coor.z,colormap='spectral')
    mlab.points3d(coor.x, coor.y, coor.z, 
                      scale_factor=0.1,colormap='spectral')
...
def makecoarsegrid():
    ...
    mlab.points3d(coor.x, coor.y, coor.z, 
                      scale_factor=0.1,colormap='spectral')
    mlab.triangular_mesh(coor.x,coor.y,coor.z,sphere.faces,colormap='spectral')
...
def make_figures(coor, fun, interp_results, error):
    ...
    mlab.mesh(coor.fine.x, coor.fine.y, coor.fine.z,
              scalars=fun.fine, vmax=vmax, vmin=vmin,colormap='spectral')
    mlab.points3d(coor.coarse.x, coor.coarse.y, coor.coarse.z, fun.coarse,
                  scale_factor=0.1, scale_mode='none', vmax=vmax, vmin=vmin,colormap='spectral')
    mlab.colorbar(
    ...
    mlab.figure(fgcolor=fgcol,bgcolor=bgcol)
    mlab.mesh(coor.fine.x, coor.fine.y, coor.fine.z,
              scalars=interp_results, vmax=vmax, vmin=vmin,colormap='spectral')
    mlab.points3d(coor.coarse.x, coor.coarse.y, coor.coarse.z, fun.coarse,
                  scale_factor=0.1, scale_mode='none', vmax=vmax, vmin=vmin,colormap='spectral')
    mlab.colorbar(
        # title='Interpolation', 
    ...
    mlab.figure(fgcolor=fgcol,bgcolor=bgcol)
    mlab.mesh(coor.fine.x, coor.fine.y, coor.fine.z,
              scalars=error.errors, vmax=error.max, vmin=-error.max,colormap='spectral')
    mlab.colorbar(
        # title='Error', 
        orientation='vertical')
...
for epsilon in epsilons:
    interp_values = rbf_interpolate(coor, function.coarse, epsilon=epsilon)
    error = interp_error(function.fine, interp_values)
    maxerror =  error.max
    errors.append(maxerror)

fig, ax = plt.subplots(facecolor=bgcol)                                                                                                                                                                                                                                                                                                                                                                
